...
export function useFarm(addNotification, addCoins, updateStats, prestigeData, buildingBonuses = {}, growthMultiplier = 1) {
  ...
  const determineQuality = useCallback((plot) => {
  ...
  // Plant a crop
  const plant = useCallback((plotIndex) => {
  ...
  // Treat pests
  const treatPest = useCallback((plotIndex) => {
    const plot = plots[plotIndex];
  ...
  // Treat disease
  const treatDisease = useCallback((plotIndex) => {
    const plot = plots[plotIndex];
  ...
  const getPlotStatus = useCallback((plotIndex) => {
    const plot = plots[plotIndex];
