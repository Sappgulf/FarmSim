...
export function FarmGrid({ 
  ...
  const getPlotEmoji = (plot) => {
    switch (plot.state) {
      case 'empty': return '.';
      case 'planted': return 'o';
      case 'growing': return 'o';
      case 'ready': return plot.seed?.emoji || '*';
      case 'withered': return 'x';
      default: return '.';
    }
  };
  ...
  const getPlotStatus = (plot) => {
    const status = [];
    if (plot.fertilizedAt > 0) status.push('F');
    if (plot.wateredAt > 0) status.push('W');
    if (plot.pesticideAt > 0) status.push('P');
    ...
    <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
          Farm Grid
          <Badge variant="secondary">{gridSize}x{gridSize}</Badge>
        </h2>
            ...
            {plot.state === 'growing' && (
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-gray-200 rounded-b">
                <div 
                  ...
                  style={{ width: `${Math.max(0, Math.min(100, (plot.progress || 0) * 100))}%` }}
                />
              </div>
            )}
      ...
      <div className="mt-4 flex items-center justify-center gap-2">
        <Button
          variant="outline"
          size="sm"
          onClick={() => onSizeChange?.(gridSize - 1)}
          disabled={gridSize <= 3}
        >
          Smaller
        </Button>
