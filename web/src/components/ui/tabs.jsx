...
export function TabsList({
  ...
  const baseClasses =
    variant === "underline"
      ? "relative items-center justify-start border-b border-slate-200/70 text-slate-500"
      : variant === "pills"
        ? "items-center justify-start rounded-full border border-slate-200/60 bg-slate-100/60 p-1 text-slate-600 shadow-[0_8px_20px_-20px_rgba(15,23,42,0.2)] backdrop-blur-sm"
        : "items-center justify-start rounded-[22px] border border-slate-200/60 bg-white/72 p-1.5 text-slate-600 shadow-[0_8px_20px_-20px_rgba(15,23,42,0.2)] backdrop-blur-sm";
    ...
    variant === "underline"
      ...
      : variant === "pills"
        ...
        : `inline-flex shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl px-3 py-2 text-sm font-semibold transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 touch-manipulation ${
            isActive
              ? "border border-emerald-100 bg-white text-emerald-700 shadow-[0_8px_18px_-18px_rgba(16,185,129,0.3)] ring-1 ring-emerald-100 scale-[1.02]"
              : "border border-transparent text-slate-600 hover:bg-white/70 hover:text-slate-900 hover:shadow-sm active:scale-95"
          }`;
