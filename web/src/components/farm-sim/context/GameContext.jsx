...
const getRandomTraitId = () =>
  CROP_TRAIT_IDS[Math.floor(Math.random() * CROP_TRAIT_IDS.length)] || null;
  ...
  const actions = useMemo(
    () => ({
      ...
      updateBuildings: (buildings) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_BUILDINGS, payload: buildings }),
      updateLivestock: (livestock) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_LIVESTOCK, payload: livestock }),
      updateFishing: (fishing) => dispatch({ type: GAME_ACTIONS.UPDATE_FISHING, payload: fishing }),
      updateAchievements: (achievements) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_ACHIEVEMENTS, payload: achievements }),
      updateSeason: (season) => dispatch({ type: GAME_ACTIONS.UPDATE_SEASON, payload: season }),
      setSeasonalEvents: (events) =>
        dispatch({ type: GAME_ACTIONS.SET_SEASONAL_EVENTS, payload: events }),
      updateActiveEvents: (events) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_ACTIVE_EVENTS, payload: events }),
      setDailyChallenges: (challenges) =>
        dispatch({ type: GAME_ACTIONS.SET_DAILY_CHALLENGES, payload: challenges }),
      updateChallengeProgress: (progress) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_CHALLENGE_PROGRESS, payload: progress }),
      updateDailyQuests: (quests) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_DAILY_QUESTS, payload: quests }),
      updateDisasterProtections: (protections) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_DISASTER_PROTECTIONS, payload: protections }),
      updateMinigames: (minigames) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_MINIGAMES, payload: minigames }),
      updatePrestige: (prestige) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_PRESTIGE, payload: prestige }),
      prestigeReset: () => dispatch({ type: GAME_ACTIONS.PRESTIGE_RESET }),
      updateResearch: (research) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_RESEARCH, payload: research }),
      updateGenetics: (genetics) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_GENETICS, payload: genetics }),
      updateSocial: (social) => dispatch({ type: GAME_ACTIONS.UPDATE_SOCIAL, payload: social }),
      updateMilestones: (milestones) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_MILESTONES, payload: milestones }),
      updateGhostVisit: (ghostVisit) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_GHOST_VISIT, payload: ghostVisit }),
      setSeedProvenance: (seedProvenance) =>
        dispatch({ type: GAME_ACTIONS.SET_SEED_PROVENANCE, payload: seedProvenance }),
      updatePets: (pets) => dispatch({ type: GAME_ACTIONS.UPDATE_PETS, payload: pets }),
      updateProcessingFacilities: (facilities) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_PROCESSING_FACILITIES, payload: facilities }),
      updateProcessingQueue: (queue) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_PROCESSING_QUEUE, payload: queue }),
      updateProcessedInventory: (inventory) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_PROCESSED_INVENTORY, payload: inventory }),
      updateChallengeStreak: (streak) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_CHALLENGE_STREAK, payload: streak }),
      updateLastChallengeReset: (timestamp) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_LAST_CHALLENGE_RESET, payload: timestamp }),
      ...
      setSelectedCrop: (cropId) =>
        dispatch({ type: GAME_ACTIONS.SET_SELECTED_CROP, payload: cropId }),
      setSelectedDecoration: (decorId) =>
        dispatch({ type: GAME_ACTIONS.SET_SELECTED_DECORATION, payload: decorId }),
      setDecorationMode: (enabled) =>
        dispatch({ type: GAME_ACTIONS.SET_DECORATION_MODE, payload: enabled }),
      updateEntitlements: (entitlements) =>
        dispatch({
          type: GAME_ACTIONS.UPDATE_ENTITLEMENTS,
          payload: normalizeEntitlements(entitlements),
        }),
      showPremiumLockPrompt: (payload) =>
        dispatch({
          type: GAME_ACTIONS.SET_PREMIUM_LOCK_PROMPT,
          payload,
        }),
      clearPremiumLockPrompt: () =>
        dispatch({
          type: GAME_ACTIONS.SET_PREMIUM_LOCK_PROMPT,
          payload: null,
        }),
      updateMemoryFlags: (memoryFlags) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_MEMORY_FLAGS, payload: memoryFlags }),
      updateMemoryCounters: (memoryCounters) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_MEMORY_COUNTERS, payload: memoryCounters }),
      updateAlmanac: (almanac) => dispatch({ type: GAME_ACTIONS.UPDATE_ALMANAC, payload: almanac }),
      updateCozyGoals: (cozyGoals) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_COZY_GOALS, payload: cozyGoals }),
      updateWhatsNew: (whatsNew) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_WHATS_NEW, payload: whatsNew }),
      updateOnboarding: (onboarding) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_ONBOARDING, payload: onboarding }),
      updateRetention: (retention) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_RETENTION, payload: retention }),
      resetOnboarding: () =>
        dispatch({
      ...
      setActiveFarmTitle: (titleId) =>
        dispatch({ type: GAME_ACTIONS.SET_ACTIVE_FARM_TITLE, payload: titleId }),
      updateSettings: (settings) =>
        dispatch({ type: GAME_ACTIONS.UPDATE_SETTINGS, payload: settings }),
      ...
      resumeGame: () =>
        dispatch({ type: GAME_ACTIONS.UPDATE_GAME_LOOP, payload: { paused: false } }),
      ...
      saveGame: () => {
        try {
          const saveResult = saveStateToStorage(stateRef.current, {
            key: SAVE_KEY,
      ...
      generateCozyGoals: (dayKey = getDayKey()) => {
        ...
        if (
          currentGoals?.dayKey === dayKey &&
          Array.isArray(currentGoals.goals) &&
      ...
      recordMilestoneEvent: (eventType, payload = {}) => {
        ...
        const nextProgress = milestoneManager.onEvent(
          eventType,
          payload,
          milestones.progress || {}
        );
        const unlockedNow = milestoneManager.evaluateUnlocks(
          nextProgress,
          milestones.unlocked || {}
        );
        ...
        unlockedNow.forEach((milestone) => {
          ...
          if (milestone.reward?.memoryId)
            actionsRef.current?.unlockMemory(milestone.reward.memoryId);
          if (milestone.reward?.almanacId)
            actionsRef.current?.unlockAlmanacPage(milestone.reward.almanacId);
      ...
      recordCozyGoalEvent: (eventType, eventData = {}) => {
        ...
        const content = getContentManager();
        let cozyState = stateRef.current.cozyGoals || {
      ...
      recordOnboardingEvent: (eventType) => {
        ...
        if (
          (eventType === 'harvest' || eventType === 'board_open') &&
          !currentState.almanac?.unlocked?.first_steps
        ) {
        ...
        if (eventType === 'harvest' && !hasFirstHarvest) {
          ...
          const bonusCoins = 20;
          const bonusRep = 1;
          const currentSocial = currentState.social || {
            friends: [],
            reputation: 0,
            marketListings: [],
          };
      ...
      recordCozyExpansionEvent: (eventType, eventData = {}) => {
        ...
        const cropTraits = current.cropTraits || {
          discoveredByCrop: {},
          totalDiscovered: 0,
          lastDiscovered: null,
        };
        ...
        const farmTitles = current.farmTitles || {
          unlocked: { home_grower: true },
          activeId: 'home_grower',
        };
        const visualState = current.visualState || {
          weather: 'sunny',
          lastPeriodKey: null,
        ...
        let next = {
          cropTraits: {
            ...cropTraits,
            discoveredByCrop: { ...(cropTraits.discoveredByCrop || {}) },
          },
          rareMoments: {
            ...rareMoments,
            unlocked: { ...(rareMoments.unlocked || {}) },
            dayKeys: { ...(rareMoments.dayKeys || {}) },
          },
          decorSets: {
            ...decorSets,
            completed: { ...(decorSets.completed || {}) },
            progress: { ...(decorSets.progress || {}) },
          },
          farmTitles: {
            ...farmTitles,
            unlocked: { ...(farmTitles.unlocked || {}), home_grower: true },
            activeId: farmTitles.activeId || 'home_grower',
          },
        ...
        const unlockTitle = (titleId) => {
          ...
          dispatch({
            type: GAME_ACTIONS.ADD_NOTIFICATION,
            payload: { message: `🏷️ New Farm Title: ${FARM_TITLES[titleId].name}`, type: 'info' },
          });
        };
        ...
        const triggerRareMoment = (momentId) => {
          ...
          dispatch({
            type: GAME_ACTIONS.ADD_NOTIFICATION,
            payload: { message: `${moment.icon} Rare Moment: ${moment.name}`, type: 'success' },
          });
        };
        ...
        if (eventType === 'crop_harvested' && eventData.cropId) {
          if (!next.cropTraits.discoveredByCrop[eventData.cropId] && rollChance(0.09)) {
            ...
            if (traitId) {
              ...
              next.cropTraits.totalDiscovered =
                Math.max(0, Number(next.cropTraits.totalDiscovered || 0)) + 1;
              next.cropTraits.lastDiscovered = {
              ...
              dispatch({
                type: GAME_ACTIONS.ADD_NOTIFICATION,
                payload: {
                  message: `${CROP_TRAITS[traitId]?.icon || '🌿'} Trait discovered: ${CROP_TRAITS[traitId]?.name || traitId}`,
                  type: 'info',
                },
              });
            }
        ...
        if (eventType === 'day_rollover') {
          ...
          if (
            getDayOfWeekIndex(dayKey) === WEEKLY_SPECIAL_DAY.dayIndex &&
            ...
            dispatch({
              type: GAME_ACTIONS.ADD_NOTIFICATION,
              payload: { message: `📌 ${WEEKLY_SPECIAL_DAY.boardCopy}`, type: 'info' },
            });
          }
        }
        ...
        if (eventType === 'decor_layout_changed') {
          const plots = Array.isArray(currentState.plots) ? currentState.plots : [];
          const placedIds = plots
            .filter((plot) => plot?.state === 'decor' && plot?.decorationId)
            .map((plot) => plot.decorationId);
          DECOR_SETS.forEach((setDef) => {
            ...
            if (placedCount >= setDef.itemIds.length && !next.decorSets.completed[setDef.id]) {
              ...
              dispatch({
                type: GAME_ACTIONS.ADD_NOTIFICATION,
                payload: { message: `🪴 Decor Set complete: ${setDef.name}`, type: 'success' },
              });
            }
          });
        }

        const unlockedPages = Object.values(currentState.almanac?.unlocked || {}).filter(
          Boolean
        ).length;
        if (unlockedPages >= 12) {
      ...
      exitGhostVisit: () => {
        dispatch({
          type: GAME_ACTIONS.UPDATE_GHOST_VISIT,
          payload: { active: false, snapshot: null },
        });
      },
      ...
      placeDecoration: (plotIndex, decorationId) => {
        ...
        if (!isItemUnlocked(currentState, decorationId, 'decor')) {
          ...
          logDebugAction('premium_locked_cosmetic', {
            decorationId,
            packId: entitlementInfo?.packId,
          });
          return 'locked';
        }
        ...
        actionsRef.current.recordMilestoneEvent('decor_set', {
          count: Object.keys(stateRef.current.cozyExpansion?.decorSets?.completed || {}).length,
        });
      ...
      removeDecoration: (plotIndex) => {
        ...
        actionsRef.current.recordCozyExpansionEvent('decor_layout_changed', {
          decorationId,
          removed: true,
        });
      ...
      plantCrop: (plotIndex, cropType, cropData) => {
        ...
        const uniqueCrops = new Set(
          plots.map((plot) => plot?.crop?.id).filter((id) => typeof id === 'string')
        );
      ...
      spendMoney: (amount, { optional = false } = {}) => {
        ...
        dispatch({
          ...
          payload: (coins) => Math.max(0, coins - tunedCost),
        });
      ...
      sellAllInventoryCrops: () => {
        const currentState = stateRef.current;
        const entries = Object.entries(currentState.inventory || {}).filter(
      ...
      harvestAllReadyCrops: () => {
        ...
        setTimeout(() => {
          ...
          dispatch({
            ...
            payload: (inv) => {
              const newInv = { ...inv };
              Object.entries(inventoryUpdates).forEach(
                ([id, amt]) => (newInv[id] = (newInv[id] || 0) + amt)
              );
              return newInv;
            },
          });
        ...
        if (planned.usedFromInventory > 0) {
          dispatch({
            ...
            payload: (inventory) => ({
              ...
              fertilizer: Math.max(
                0,
                Math.floor(Number(inventory?.fertilizer || 0)) - planned.usedFromInventory
              ),
            }),
          });
        }
        ...
        if (planned.usedFromInventory > 0) {
          dispatch({
            ...
            payload: (inventory) => ({
              ...
              pesticide: Math.max(
                0,
                Math.floor(Number(inventory?.pesticide || 0)) - planned.usedFromInventory
              ),
            }),
          });
        }
  ...
  return (
    <GameStoreContext.Provider value={storeRef.current}>
      <GameActionsContext.Provider value={actions}>
        <GameSystemsContext.Provider value={systems}>{children}</GameSystemsContext.Provider>
