...
const PerformanceOverlay = memo(() => {
  ...
  const plotCount = useGameSelector((state) =>
    Array.isArray(state.plots) ? state.plots.length : 0
  );
    ...
    <>
        ...
        <div className="fixed inset-4 sm:inset-8 z-[10000] overflow-auto rounded-2xl border border-red-500/70 bg-black/90 p-4 text-white shadow-2xl">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={clearDebugError} className="text-xs text-gray-300 hover:text-white">
                Dismiss
              </button>
            </div>
          </div>
          <div className="space-y-2 text-xs">
            <div>
              <span className="text-gray-400">Source:</span> {debugInfo.lastError.source}
            </div>
            <div>
              <span className="text-gray-400">Time:</span> {debugInfo.lastError.time}
            </div>
      ...
      <div className="fixed top-2 left-2 bg-black/90 backdrop-blur-sm text-white font-mono text-xs p-3 rounded-lg z-[9999] min-w-[240px] shadow-xl border border-gray-700">
        ...
        <div className="space-y-1">
          ...
          <div className="flex justify-between">
            <span>Buildings:</span>
            <span className="text-gray-300">{metrics.buildings}</span>
          </div>

          <div className="flex justify-between">
            <span>Decor:</span>
            <span className="text-gray-300">{metrics.decorations}</span>
          </div>

          <div className="flex justify-between">
            <span>Notifs:</span>
            <span className="text-gray-300">{metrics.notifications}</span>
          </div>

          <div className="flex justify-between">
            <span>Timers:</span>
            <span className="text-gray-300">{metrics.timers}</span>
          </div>

          <div className="flex justify-between">
            <span>Listeners:</span>
            <span className="text-gray-300">{metrics.listeners}</span>
          </div>
