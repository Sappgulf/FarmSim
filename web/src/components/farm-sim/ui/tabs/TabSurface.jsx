...
export const TabHero = memo(({ icon, title, description, tone = 'slate', badge, actions, children, className = '' }) => {
  ...
  return (
    <Card className={`overflow-hidden border ${theme.border} ${className}`}>
      ...
      {children ? <div className="border-t border-white/70 bg-white/70 px-4 py-4">{children}</div> : null}
    </Card>
  );
});
...
export const MetricTile = memo(({ label, value, hint, tone = 'slate', icon, className = '' }) => {
  ...
  return (
    <div className={`rounded-2xl border border-white/70 bg-white/90 px-3 py-3 shadow-sm ${className}`}>
