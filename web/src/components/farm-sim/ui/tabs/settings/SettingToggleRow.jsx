...
export const SettingToggleRow = memo(
  ({ title, description, enabled, onToggle, label, className = '' }) => {
    return (
      <div
        className={`flex items-start justify-between gap-4 rounded-2xl border border-slate-200/70 bg-white/75 px-4 py-3 shadow-sm ${className}`}
      >
        <div className="min-w-0">
          <div className="font-medium text-slate-900">{title}</div>
          {description && <div className="text-sm leading-snug text-slate-600">{description}</div>}
        </div>
