    ...
    offers: [
      {
        ...
        want: 'sunflower',
        wantAmount: 5,
        label: 'Buy 5 Sunflowers for 150🪙',
      },
      {
        give: 'coins',
        giveAmount: 200,
    ...
    offers: [
      {
        ...
        want: 'strawberry',
        wantAmount: 10,
        label: 'Buy 10 Strawberries for 300🪙',
      },
      {
        give: 'coins',
        giveAmount: 500,
        want: 'watermelon',
        wantAmount: 5,
        label: 'Buy 5 Watermelons for 500🪙',
      },
    ],
  },
];
...
const getCommunityChallenge = (state) => {
  ...
  if (totalCrops < 20) {
    challenges.push({
      ...
      complete: totalCrops >= 20,
    });
  }
  if (petsCount < 2) {
    challenges.push({
      id: 'adopt_2',
      title: 'Companion Outreach',
  ...
  if (challenges.length === 0) {
    challenges.push({
      ...
      progress: 100,
      reward: { type: 'reputation', amount: 0 },
      complete: true,
    });
  }
  return challenges;
...
const SocialTab = memo(() => {
  ...
  const challenges = useMemo(
    () => getCommunityChallenge(state),
    [state.inventory, state.pets, state.processingFacilities, state.level]
  );

  const nextMilestones = useMemo(
    () =>
      MILESTONE_DEFINITIONS.filter((definition) => !milestones.unlocked?.[definition.id]).slice(
        0,
        3
      ),
    [milestones.unlocked]
  ...
  const handleStartFromSeed = () => {
    ...
    actions.addNotification({
      message: 'Seed start applied. Reset farm to begin fresh from this vibe.',
      type: 'info',
    });
  };
  ...
  const handleImportSnapshot = () => {
    try {
      ...
      if (!validation.ok) {
        actions.addNotification({
          message: validation.errors[0] || 'Snapshot invalid.',
          type: 'error',
        });
        return;
      }
  ...
  const progressToNext = nextTier
    ? Math.min(
        100,
        Math.round(
          ((reputation - currentTier.minRep) / (nextTier.minRep - currentTier.minRep)) * 100
        )
      )
    : 100;
    ...
    <div className="space-y-4">
        ...
        {nextTier && (
          <div className="mt-3">
            <div className="mb-1 flex justify-between text-xs text-slate-500">
              ...
              <span>
                {nextTier.name} ({nextTier.minRep} rep)
              </span>
            </div>
      ...
      >
        <div className="space-y-3">
          {visitors.map((visitor) => (
            <Card
              key={visitor.id}
              className="overflow-hidden border-amber-200/70 bg-white/90 p-3 shadow-sm"
            >
              ...
              <div className="space-y-2">
                {visitor.offers.map((offer, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="flex flex-col justify-between gap-2 rounded-2xl border border-white/80 bg-white/85 p-2 shadow-sm sm:flex-row sm:items-center"
                    >
                      <div className="text-sm">
                        <span className="font-medium">{offer.giveAmount}🪙</span>
                        <span className="text-slate-500"> for </span>
                        <span className="font-medium">
                          {offer.wantAmount} {formatDisplayLabel(offer.want)}
                        </span>
                        <span className="ml-1 text-xs text-slate-400">
                          (have {state.inventory[offer.want] || 0})
                        </span>
                      </div>
                      <Button
                        onClick={() => handleTrade(visitor, offer)}
                        size="sm"
                        disabled={done || !hasEnough}
                        className="shrink-0"
                      >
                        {done ? '✓ Done' : 'Trade'}
                      </Button>
        ...
        <div className="space-y-3">
          {challenges.map((challenge) => {
            ...
            return (
              <Card
                key={challenge.id}
                className={`overflow-hidden p-3 shadow-sm ${challenge.complete ? 'border-emerald-200/70 bg-emerald-50/80' : 'border-slate-200/70 bg-white/90'}`}
              >
      ...
      >
        <div className="flex gap-2">
          ...
          <Button
            ...
            className="flex-1 min-h-[44px]"
          >
            Start from Seed Code
          </Button>
        </div>
        <textarea
          value={seedCodeInput}
          onChange={(e) => setSeedCodeInput(e.target.value)}
          className="mt-2 w-full rounded-2xl border border-slate-200 bg-white/90 p-3 text-xs shadow-sm focus:border-emerald-300 focus:outline-none"
          rows={3}
          placeholder="Paste Seed Code or Snapshot JSON"
        />
      </TabSection>
      ...
      >
        ...
        {state.ghostVisit?.active && (
          <Button onClick={actions.exitGhostVisit} size="sm" className="w-full min-h-[44px]">
            Exit Ghost Visit
          </Button>
        )}
      </TabSection>
          ...
          {(milestones.recent || []).slice(-3).reverse().length === 0 ? (
            ...
            (milestones.recent || [])
              ...
              .map((id) => {
                ...
                return (
                  <div key={id} className="text-xs text-emerald-700">
                    Unlocked: {def?.name || id}
                  </div>
                );
              })
          )}
            ...
            return (
              <div key={definition.id}>
                <div className="text-xs text-gray-700">
                  {definition.name} ({value}/{definition.target})
                </div>
        ...
        <div className="space-y-2">
          {REPUTATION_TIERS.map((tier) => {
            ...
            return (
              <div
                key={tier.tier}
                className={`flex items-center justify-between rounded-2xl border p-2 ${unlocked ? 'border-slate-200/70 bg-white/90 shadow-sm' : 'border-slate-200/60 bg-slate-100/70 opacity-60'}`}
              >
