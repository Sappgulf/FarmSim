...
const GameHeader = memo(({ onFocusGameplay }) => {
  ...
  const weatherForecast = useGameSelector((state) =>
    Array.isArray(state.weatherForecast) ? state.weatherForecast : []
  );
        ...
        <div className="flex items-center justify-between lg:justify-start gap-2 sm:gap-4 lg:gap-6 w-full lg:w-auto">
          ...
          <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 flex-wrap justify-end lg:justify-start">
            ...
            >
              <Badge
                variant="outline"
                className="bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 border-green-200 font-bold hover:bg-emerald-100/80 transition-colors cursor-pointer"
              >
                Level {level}
              </Badge>
            </button>
        ...
        <div className="w-full lg:w-auto flex flex-wrap items-center justify-between sm:justify-end gap-2">
          ...
          <div className="relative" ref={statsDropdownRef}>
            ...
            >
              ...
              <ChevronDown
                className={`w-3 h-3 transition-transform ${showStatsDropdown ? 'rotate-180' : ''}`}
              />
            </Button>
            ...
            {showStatsDropdown && (
              <div
                ...
                className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-xl z-50 animate-overlay-card"
                role="dialog"
          ...
          {season?.config && (
            ...
            >
              <span className="text-lg animate-season drop-shadow-sm">{season.config.emoji}</span>
              <span className="text-sm font-medium text-purple-700 capitalize">
                {season.config.name}
              </span>
              {/* Season time remaining tooltip */}
              <div
                className="absolute top-full right-0 mt-2 w-48 bg-white rounded-lg p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 backdrop-blur-lg bg-opacity-95"
                style={{
              ...
              >
                ...
                <div className="text-xs text-gray-600 mb-2">{season.config.description}</div>
          ...
          >
            <span className="text-lg animate-weather">{weatherMeta.emoji}</span>
            <span className="text-xs sm:text-sm font-medium">{weatherMeta.label}</span>
          </button>
