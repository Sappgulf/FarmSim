...
export const LIVESTOCK_TYPES = {
  ...
  PIG: {
    ...
    products: [
      { item: 'truffle', amount: 1, value: 30 }
    ],
    maxHealth: 100,
    happinessDecay: 2.5,
    hungerRate: 6,
...
export class LivestockSystem {
  ...
  update(currentState) {
    ...
    const updatedAnimals = this.gameState.livestock.animals.map(animal => {
      ...
      if (timeSinceLastProduction >= animal.type.productionTime && newHealth > 50 && newHappiness > 30) {
        hasNewProduct = true;
  ...
  buyAnimal(typeId) {
    ...
    const currentSpace = this.gameState.livestock?.animals?.reduce((sum, a) => sum + a.type.requirements.space, 0) || 0;
  ...
  getStats() {
    ...
    return {
      ...
      avgHealth: animals.length > 0 ? animals.reduce((sum, a) => sum + a.health, 0) / animals.length : 0,
      avgHappiness: animals.length > 0 ? animals.reduce((sum, a) => sum + a.happiness, 0) / animals.length : 0,
